...
void tpcleader_handle_get(tpcleader_t *leader, kvrequest_t *req, kvresponse_t *res) {
  ...
  int sockfd;
  int ret;
  kvresponse_t *response;

  /* pick a follower server to work with and obtain sockfd of it */
  tpcfollower_t *tpcfollower = tpcleader_get_primary(leader, req->key);
  sockfd = connect_to(tpcfollower->host, tpcfollower->port, 10);

  /* send GET request to the follower server */
  ret = kvrequest_send(req, sockfd);

  /* recieve response from the follower server */
...
void tpcleader_handle_tpc(tpcleader_t *leader, kvrequest_t *req, kvresponse_t *res) {
  ...
  tpcfollower_t *primary_tpcfollower = tpcleader_get_primary(leader, req->key);
  tpcfollower_t *curr_tpcfollower;

  /* polling r servers */
  curr_tpcfollower = primary_tpcfollower;

  for (counter = 0; counter < leader->redundancy; counter++) {
    /* pick a follower server to work with and obtain sockfd of it */
    sockfd = connect_to(curr_tpcfollower->host, curr_tpcfollower->port, 10);
    if (sockfd == -1) {
      break;
    }
    ...
    /* recieve VOTE from the follower server */
    response = kvresponse_recieve(sockfd);
    if (response != NULL) {
      if (strcmp(response->body, MSG_COMMIT) != 0) {
        break;
      }
    } else {
      break;
    ...
    curr_tpcfollower = tpcleader_get_successor(leader, curr_tpcfollower);
  }
  ...
  /* set the request */
  if(counter == leader->redundancy) { /* there was no ABORT */
    /* send out COMMIT */
    req->type = COMMIT;
  } else {  /* there was ABORT */
    /* send out ABORT */
    req->type = ABORT;
  }


  /* send out request to r servers */
  curr_tpcfollower = primary_tpcfollower;
